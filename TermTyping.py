...
if LLM_MODEL == "Qwen":
    ...
    def generate_qwen_batched(prompts, llm_model, tokenizer_model, generation_cfg):
        ...
        # On prépare tout le batch d'un coup
        inputs = tokenizer_model.apply_chat_template(
            turns_batch,
            add_generation_prompt=True,
            return_tensors="pt",
            padding=True # Important pour le batching !
        ).to(device)

        # Les GPUs se partagent le travail ici
        generated_ids = llm_model.module.generate( # .module requis avec DataParallel
            inputs,
            max_new_tokens=generation_cfg.max_new_tokens
        ) if n_gpus > 1 else llm_model.generate(
            inputs,
            max_new_tokens=generation_cfg.max_new_tokens
        )
    ...
    def generate_google_batched(prompts, llm_model, tokenizer_model, generation_cfg):
        ...
        # Support DataParallel vs Single GPU
...
def run_classification(classification_task):
    if LLM_MODEL == "Google":
    ...
    # ... (le début de run_classification reste pareil) ...
...
def classify_term_type_with_rag(term, sentence, labels, llm_model, tokenizer_model, few_shot_prompting, active_generate_func, generation_cfg, embedder, wikidata_embeddings, wikidata_sentences):
    """
    ...
    retrieved_context = get_rag_context(sentence, embedder, wikidata_embeddings, wikidata_sentences, k=1)
...
def classify_term_type_with_dynamic_few_shot(term, sentence, labels, llm_model, tokenizer_model, active_generate_func, generation_cfg, embedder, train_embeddings, train_terms, train_labels, train_sentences, id2label):

    # 1. Récupération intelligente (on passe aussi le terme)
    dynamic_examples = get_dynamic_few_shot_examples(sentence, term, embedder, train_embeddings, train_terms, train_labels, train_sentences, id2label, k=1)
...
if __name__ == "__main__":
    run_classification("classify_term_type_with_dynamic_few_shot")